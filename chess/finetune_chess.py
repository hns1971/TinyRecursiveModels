...
示例:
  ...
  python chess/finetune_chess.py \\
    ...
    parser.add_argument("--global-batch-size", type=int, default=None,
                       help="全局批次大小（默认：使用配置文件中的值）。如果不指定，将使用cfg_finetune_chess.yaml中的值")
    
    # 配置/架构参数
    parser.add_argument("--config-name", type=str, default="cfg_finetune_chess",
                       help="Hydra 配置名（默认：cfg_finetune_chess）")
    ...
    parser.add_argument("--lm-loss-weight", type=float, default=None,
                       help="lm_loss的权重（默认：使用配置文件中的值）。如果不指定，将使用cfg_finetune_chess.yaml中的值")
    parser.add_argument("--q-halt-loss-weight", type=float, default=None,
                       help="q_halt_loss的权重（默认：使用配置文件中的值）。如果不指定，将使用cfg_finetune_chess.yaml中的值")
    ...
    cmd = [
        ...
        "evaluators=[]",  # 象棋任务不需要特殊评估器（或可以后续添加）
        # 使用ACTLossHead（象棋任务使用标准损失函数）
        f"arch.loss.name=losses@ACTLossHead",
